...
export function DealDisplay({ deal }: DealDisplayProps) {
  return (
    <Card className="w-full max-w-5xl mx-auto overflow-hidden mt-6 shadow-lg">
         ...
         <div className="flex flex-col md:flex-row items-start gap-4">
          {deal.Image1 ? (
            <Image
              ...
              width={100}
              height={100}
              className="rounded-md border object-cover aspect-square flex-shrink-0"
              data-ai-hint="product logo brand"
            />
          ) : (
             <div className="w-[100px] h-[100px] bg-secondary rounded-md flex items-center justify-center flex-shrink-0">
                <ImageIcon className="w-10 h-10 text-muted-foreground" />
             </div>
          )}
          <div className="flex-grow">
             <CardTitle className="text-2xl font-bold mb-1">{deal["Deal Title"]}</CardTitle>
